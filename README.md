...
<head>
  ...
  <title>Veux-tu être mon/ma Valentin(e) ?</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    #ouiBtn:hover { background: var(--yesHover); }
...
<body>
  ...
  <main class="card">
    ...
    <h1>Hugo mon cœur, je veux plus jamais te prendre la tête, je veux juste me poser avec toi alors veux-tu être mon Valentin 💐 ?</h1>
    ...
    <!-- HINT -->
    <div class="hint" id="hint">“Non” a l’air un peu timide 😈</div>
  ...
  <script>
    ...
    let ouiScale = 1;
    function growOui() {
      ouiScale = Math.min(2.2, ouiScale + 0.1);
      ouiBtn.style.transform = `translateY(-50%) scale(${ouiScale})`;
    }

    /* ---------- NON BUTTON RUNS AWAY ---------- */
    function clamp(n, min, max) {
      return Math.max(min, Math.min(max, n));
    }

    function moveNon(px, py) {
      const z = zone.getBoundingClientRect();
      const b = nonBtn.getBoundingClientRect();
      ...
      nonBtn.style.left = newLeft + "px";
      nonBtn.style.top = newTop + "px";
    ...
    zone.addEventListener("pointermove", e => {
      ...
      if (d < 140) moveNon(e.clientX, e.clientY);
    });

    nonBtn.addEventListener("click", e => e.preventDefault());
